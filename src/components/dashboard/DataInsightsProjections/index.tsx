...
const DataInsightsProjections: React.FC<DataInsightsProjectionsProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const dataPromises = realEstateCompanies.map(async (company) => {
          // Get sentiment data
          const sentiment = await marketSentiment.getSentiment(company.ticker);
  ...
  // Use real-time data if available, otherwise fall back to the provided data
  const displayData = realTimeData || realEstateCompanies;
